...
class OrderSyncScheduler {
    constructor(config = {}) {
        ...
        this.nextScheduledRun = null; // Store the actual next run time
        this.metrics = {
            totalSyncs: 0,
            successfulSyncs: 0,
            failedSyncs: 0,
    ...
    async start() {
        ...
        // Calculate and store the initial next run time
        ...
        this.task = cron.schedule(this.config.interval, async () => {
            // Update next run time for the following execution
            this.updateNextRunTime();
            await this.runSync();
        });
    ...
    async runSync() {
        ...
        try {
            ...
            if (lastSync && typeof lastSync === 'string') {
                ...
                lastSync = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000); // Default to 7 days ago
            }
    ...
    updateNextRunTime() {
        ...
        try {
            // Parse the cron expression and calculate next run from now
            const cronParser = require('cron-parser');
            const interval = cronParser.parseExpression(this.config.interval);
            this.nextScheduledRun = interval.next().toDate();
        } catch (error) {
            // Fallback: estimate based on interval pattern
            const match = this.config.interval.match(/\*\/(\d+)/);
            if (match) {
                const minutes = parseInt(match[1]);
                this.nextScheduledRun = new Date(Date.now() + minutes * 60 * 1000);
            } else {
                this.nextScheduledRun = null;
            }
        }
    ...
    getNextRunTime() {
        // Return the stored next run time instead of recalculating
    ...
    async getStatus() {
        ...
        let nextRunFormatted = null;
        if (metrics.nextRun) {
            try {
                nextRunFormatted = metrics.nextRun instanceof Date 
                    ? metrics.nextRun.toISOString() 
                    : metrics.nextRun;
            } catch (e) {
                nextRunFormatted = 'Unable to determine';
            }
        }
