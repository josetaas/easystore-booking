        ...
        for (const item of order.line_items) {
            ...
            if (bookingDate && bookingTime) {
                bookings.push({
                    ...
                    productName: item.product_name || item.name || item.title || 'Unknown Product',
    ...
    async processOrder(order, options = {}) {
        ...
        try {
            ...
            if (!options.force) {
                ...
                console.log(`🔍 Checking if order ${orderId} (${order.order_number}) is already processed...`);
                const isProcessed = await dataAccess.isOrderProcessed(orderId);
                
                if (isProcessed) {
                    const existingOrder = await dataAccess.getProcessedOrder(orderId);
                    console.log(`⚠️  Order ${orderId} (${order.order_number}) already processed:`, {
                        processedAt: existingOrder?.processed_at,
                        calendarEventId: existingOrder?.calendar_event_id,
                        syncSource: existingOrder?.sync_source
                    });
                    ...
                    console.log(`✅ Order ${orderId} (${order.order_number}) not yet processed, proceeding...`);
                }
            }
            ...
            if (results.success) {
                for (const booking of bookings) {
                    const calendarEventId = results.bookings.find(b => b.lineItemId === booking.lineItemId)?.calendarEventId;
                    console.log(`📝 Marking order ${booking.orderId} (${booking.orderNumber}) as processed with calendar event: ${calendarEventId}`);
                    
                    try {
                        await dataAccess.markOrderProcessed(booking.orderId, {
                        ...
                        console.log(`✅ Successfully marked order ${booking.orderId} as processed`);
                    } catch (dbError) {
                        console.error(`❌ Failed to mark order ${booking.orderId} as processed:`, dbError);
                        throw dbError;
                    }
                }
